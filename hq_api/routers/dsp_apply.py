...
def apply_audio(config: AudioConfig, bt: BackgroundTasks):
    ...
    try:
        ...
        if config.mode == "dsp":
            # NOTE: 2026-09-05 修正 — ユーザーの現在の volume (config.volume) を尊重し、
            # last_config の古い volume で上書きしない。
            # 以前は saved_volume = last_config.volume を使用していたが、
            # /api/volume ダイヤル操作直後に /api/apply が走ると volume がリセットされる
            # 「ボリュームダイヤルに触れないと音が出ない」症状の原因。
            if needs_restart:
