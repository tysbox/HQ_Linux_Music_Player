...
export function SoundgenicView({ currentUri, onAddToPlaylist }: Props) {
  ...
  const browseRoot = useCallback(async (sid: ServerId = activeServer) => {
    ...
    const rootLabel = sid === 'soundgenic' ? 'Soundgenic' : servers.find(s => s.id === sid)?.name ?? 'Asset UPnP'
    setBreadcrumbs([{ id: '0', label: rootLabel }])
  ...
  const browseTo = async (id: string, label: string, crumbs: { id: string; label: string }[]) => {
    setLoading(true); setIsSearching(false)
    setBreadcrumbs([...crumbs, { id, label }])
    try { const d = await upnpApi.browse(id, activeServer); setContainers(d.containers); setTracks(d.tracks) }
    catch { setContainers([]); setTracks([]) }
    setLoading(false)
  }

  const browseContainer = async (c: UPnPContainer) => {
    await browseTo(c.id, c.title, breadcrumbs)
  }

  const goCrumb = async (index: number) => {
    const target = breadcrumbs[index]
    if (!target) return
    setLoading(true); setIsSearching(false)
    setBreadcrumbs(breadcrumbs.slice(0, index + 1))
    try { const d = await upnpApi.browse(target.id, activeServer); setContainers(d.containers); setTracks(d.tracks) }
    catch { setContainers([]); setTracks([]) }
    setLoading(false)
  }

  const currentContainerId = breadcrumbs.length > 0 ? breadcrumbs[breadcrumbs.length - 1].id : '0'

  useEffect(() => {
    const t = setTimeout(async () => {
      if (!q.trim()) { setIsSearching(false); return }
      setIsSearching(true); setLoading(true)
      try { const d = await upnpApi.search(q, activeServer, currentContainerId); setSearchRes(d.tracks) }
      catch { setSearchRes([]) }
      setLoading(false)
    }, 400)
    return () => clearTimeout(t)
  }, [q, activeServer, currentContainerId])

  const playTrackNow = async (t: Track) => {
    if (!t.uri) return
    setBusyId(t.id)
    try {
      await api.queue.add(t.uri, true, false, {
        title: t.title,
        artist: t.artist,
        album: t.album,
  ...
  const playAll = async (tl: Track[]) => {
    const playable = tl.filter(t => t.uri)
    if (playable.length === 0) return
    setBusyId('__all__')
    try {
      await api.queue.clear()
      for (const t of playable) await api.queue.add(t.uri, false, false, {
        title: t.title,
        artist: t.artist,
        album: t.album,
        artwork_url: t.artwork_url,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Server selector — 使用中サーバーのみ表示、前画面でサーバー選択一覧 */}
      <div style={{
        ...
        <button
          ...
          style={{ background: 'none', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 4, color: 'rgba(255,255,255,0.60)', fontSize: 8, padding: '4px 8px', cursor: 'pointer', flexShrink: 0 }}
        >
          {showServers ? '▲ SERVERS' : '▽ SERVERS'}
        </button>
      </div>

      {/* Server selection list (previous screen) */}
        ...
        <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '0 6px 8px' }}>
            {servers.map(srv => (
              <button
                key={srv.id}
                onClick={() => { setActiveServer(srv.id as ServerId); setShowServers(false); browseRoot(srv.id as ServerId) }}
                style={{
                  display: 'flex', alignItems: 'center', gap: 10, textAlign: 'left',
                  padding: '9px 10px', background: srv.id === activeServer ? 'rgba(34,197,94,0.08)' : 'none',
                  border: srv.id === activeServer ? '1px solid rgba(34,197,94,0.30)' : '1px solid transparent',
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {/* Search results — 現在フォルダ内 */}
        {!loading && isSearching && (
          <div className="fade-in">
            <div style={{ padding: '9px 14px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              <span className="engraved">{searchRes.length} results for "{q}" in {breadcrumbs.length > 0 ? breadcrumbs[breadcrumbs.length - 1].label : activeInfo?.name}</span>
            </div>
        ...
        {!loading && !isSearching && !isOffline && (
          <div className="fade-in">
            {/* Containers */}
            {containers.map(c => (
              <button key={c.id} onClick={() => browseContainer(c)} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.05)', borderTop: containers.length > 0 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                ...
                {containers.length === 0 && (
                  <button className="touch-sw touch-sw-green" style={{ height: 22, padding: '0 10px', gap: 5, borderRadius: 4 }}
                    ...
                    <span className="led-green" style={{ fontSize: 9 }}>{busyId === '__all__' ? '…' : '▶'}</span>
